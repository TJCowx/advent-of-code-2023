...
fn solve_sequence(nums: Vec<i32>) -> i32 {
    ...
    println!("{:?}", diffs);
    for (i, diff) in diffs.iter_mut().rev().enumerate() {
        // ONLY CHANGES FROM PART 1
        // Sum = the first number in the next array - the sum of the current array
        sum = diff.first().unwrap() - sum;

        println!("{}: {}", i, sum);
        // Then we just push it to the front of the array instead of the back
        diff.insert(0, sum);
    }
